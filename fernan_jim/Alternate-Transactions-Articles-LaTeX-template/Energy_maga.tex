...
Data mining is the process of extraction of relevant
information from a collection of data. Mining of a particular
information related to a concept is done on the basis of the
feature of the data. The accessing of these features hence for data
retrieval can be termed as the feature extraction mechanism.
...
A \textbf{feature} is an individual measurable property or characteristic of a phenomenon being observed. 
...
FS facilitates data understanding, reduces the measurement and storage requirements, reduces the computational process time, and reduces the size of a data set,  so that model learning becomes an easier process. 
An FS method is basically a \textit{search strategy} where the performance of candidate subsets is measured with a given  \textit{evaluator}. The search space for candidate subsets has cardinality $O(2^w)$, where $w$ is the number of features. A \textit{stopping criterion} establishes when the feature selection process must finish. It can be defined as a control procedure
that ensures that no further addition or deletion of features does
produce a better subset, or it can be as simple as a counter of iterations. 
...
\begin{itemize}
\item \textit{Wrapper methods} \cite{kohavi1997wrappers} use a predetermined learning algorithm to determine the quality of selected features according to an evaluation metric \cite{Japkowicz:2011:ELA:1964882}.
\item \textit{Filter methods} apply statistical measures to evaluate the set of attributes \cite{Kar10, Hal98, Ami05}
\item \textit{Embedded methods} achieve model fitting and feature selection simultaneously \cite{Sal94}.
\item  \textit{Multivariate methods} evaluate features in batches.
\item  \textit{Univariate methods} evaluate each feature independently.
\end{itemize}
