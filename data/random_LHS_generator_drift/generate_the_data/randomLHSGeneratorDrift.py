...
#    features = [2,3,4,5]
    features = [100] # los atributos
    file_names = ["LHSGeneratedK100F%i" % f for f in features]
#    limit = 100000
    limit = 192
    for f, fname in zip(features, file_names):
#        stream = RandomLHSGeneratorDrift(file_name=fname, sample_limit=limit, center_num=10, dimensions=f)
        stream = RandomLHSGeneratorDrift(file_name=fname, sample_limit=limit, center_num=10, dimensions=f, distribution= "gauss")
