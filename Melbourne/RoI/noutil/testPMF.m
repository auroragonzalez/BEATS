...
for k = NAS
    tic
    [R2, elements] = pmf3(locations, k, R, max_epoch, rank, lr, mom,reg, minibatch_size,norm);
...
for k = NAS
    tic
    [R2, elements] = pmf3(locations, k, Rgt, max_epoch, rank, lr, mom,reg, minibatch_size,norm);
    realTest = Rgt(elements);
    predTest = R2(elements);
