...
X_s = X.tolil()
#while epoch < max_epoch:  #xxx
epoch=1
# Get indices for non-NaN values
r, c = X.nonzero()

XX = zip(r, c)
minibatch_indices = np.arange(0, len(XX), minibatch_size)
minibatch_indices = np.asarray(list(minibatch_indices) + [len(XX)])
start_indices = minibatch_indices[:-1]
end_indices = minibatch_indices[1:]
mb_indices = zip(start_indices, end_indices)
...
import pandas as pd
df=pd.read_csv('RoI.csv', sep=',',header=None)
R = df.values
U, V, m = PMF(R, learning_rate=0.001, momentum=0.95,
              minibatch_size=2, rank=5, max_epoch=1000, random_state=1999)
R2 = np.dot(U, V.T) + m


R = df.values
a = R
row_idx = np.array(range(0,3))
col_idx = np.array(range(0,1200))
b = a[row_idx[:, None], col_idx]
R = b
U, V, m = PMF(R, learning_rate=0.001, momentum=0.8,
              minibatch_size=2, rank=10, max_epoch=1000, random_state=1999)
R2 = np.dot(U, V.T) + m

import numpy
numpy.savetxt("R2.csv", R2, delimiter=",")
