...
def score_NN_measure(actual, predicted):
    diff_m = abs(actual - predicted)
    total_l = len(actual)
    pred_l = len(diff_m[diff_m <= 0.3])
    return (pred_l*100.00)/total_l, pred_l, total_l
...
if __name__ == "__main__":
    ...
    for ind in range(3,21):# change it into 21
        ...
        for index in range(1, k+1):
            ...
            if len(data) > 100:
                ...
                testing_SVM_NN_class_organized = data_into_class(testing_SVM_NN_class)

                start_time=time.time()
                NN_classification = DeepNN.MLPClassifier()
                NN_classification.fit(np.array(training_SVM_NN).astype(float),training_SVM_NN_class_organized)
                elapsed_time_train=time.time()-start_time
                start_time2 = time.time()

                result_from_NN = NN_classification.predict( np.array(testing_SVM_NN).astype(float))
                elapsed_time_estimate=time.time()-start_time2

                out_RMSE, out_max, out_min, out_avg = rmse_class(np.array(testing_SVM_NN_class_organized).astype(float),result_from_NN)
                score_NN1, pred_num, pred_tot = score_NN_measure((np.array(testing_SVM_NN_class_organized).astype(float)), np.array(result_from_NN).astype(float))

                rmse_all.append([k, index, out_RMSE, out_max, out_min, out_avg, score_NN1, pred_num, pred_tot])
    ...
    if len(rmse_all) > 0:
        ...
        R_hat_NN.to_csv('data_other/June_13/All_Result_DNN_2_RMSE_Acc.csv', index=False)

    if len(time_list) > 0:
        lst_time = pd.DataFrame(np.array(time_list).T)
        lst_time.to_csv('data_other/June_13/All_Result_DNN_2_elapsed_time.csv', index=False)
