...
for ind in range(3, 21):  # range(3,21): # change it into 21
    k = ind
...
#        print "K : ", k, " Index: ", index, " Data amount: ", data.shape
        if (data.shape[1]> 1): # cuando haya más de una columna en el grupo (más de un sensor claro)
            ...
            CVRMSE_now = RMSE_now/np.mean(real)*100
            MAPE.append([k, index, MAPE_now])
            RMSE.append([k, index, RMSE_now])
            CVRMSE.append([k, index, CVRMSE_now])
            ...
            pd.DataFrame(np.transpose([[k]*len(sensRep),sensRep, trreal, trpred])).to_csv('/home/aurora/Git_repos/posgradoActual/Melbourne/BMECode/metrics/predandRealPMF.csv', index=False,mode='a')


            exact_acc, five_acc, thirty_acc = accuracy_percent_pmf(build_matrix_from_tuple(col_num, row_num, validation), R_hat)
            time_list.append([k, index, elapsed_time_all])
            ACC01.append([k, index, exact_acc])
            realFinal.append(real)
            predictedFinal.append(predicted)

        else:
            MAPE.append([k, index, -999])
            RMSE.append([k, index, -999])
            CVRMSE.append([k, index, -999])
            time_list.append([k, index, -999])
            ACC01.append([k, index, -999])
...
ACC01 = pd.DataFrame(np.array(ACC01).T)
ACC01.to_csv('/home/aurora/Git_repos/posgradoActual/Melbourne/BMECode/metrics/Accuracy.csv', index=False)
MAPE = pd.DataFrame(np.array(MAPE).T)
MAPE.to_csv('/home/aurora/Git_repos/posgradoActual/Melbourne/BMECode/metrics/MAPE.csv', index=False)
RMSE = pd.DataFrame(np.array(RMSE).T)
RMSE.to_csv('/home/aurora/Git_repos/posgradoActual/Melbourne/BMECode/metrics/RMSE.csv', index=False)
time_list = pd.DataFrame(np.array(time_list).T)
time_list.to_csv('/home/aurora/Git_repos/posgradoActual/Melbourne/BMECode/metrics/time.csv', index=False)
CVRMSE = pd.DataFrame(np.array(CVRMSE).T)
CVRMSE.to_csv('/home/aurora/Git_repos/posgradoActual/Melbourne/BMECode/metrics/CVRMSE.csv', index=False)
