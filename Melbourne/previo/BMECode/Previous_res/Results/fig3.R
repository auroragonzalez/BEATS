df <- read.table("fig3.csv", sep=",", head=F)
...
xlab ="# Observations per sensor (fixed nh = 8)", ylab="", main="BME with raw temp IBRL data")

dev.off()
svg(filename="fig3-opt1.svg", width = 12, height = 8, pointsize = 12)
xyplot(value~V1|variable, data = dat.m, layout = c(1,(ncol(df))-1), lwd=2, pch=19, type="b",scales = list(y = list(relation = "free", rot=0), x = list(at= seq(120,1200,120))),
       xlab ="# Observations per sensor (fixed nh = 8)", ylab="", main="BME with raw temp IBRL data")
dev.off()
...
plot(df$V1, df$MAPE,ylab ="MAPE (%)", xlab="# Observations per sensor (fixed nh = 8)"
...
dev.off()
svg(filename="fig3-opt2.svg", width = 12, height = 8, pointsize = 12)
plot(df$V1, df$MAPE,ylab ="MAPE (%)", xlab="# Observations per sensor (fixed nh = 8)"
     , main="BME with raw temp IBRL data", ylim = c(3,4.5)
     , col="#00994C", pch = 19, xaxt="n")
axis(1, at = seq(120,1200,120))
