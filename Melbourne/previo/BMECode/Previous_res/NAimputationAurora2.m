...
positions2 = positions.*matrixForNA;
%add the 3 that are all missing and useless (future purposes)
kk = 4
positions2 = [positions2(1:kk,:); zeros(1,N); positions2(kk+1:end,:)]
kk = 14
positions2 = [positions2(1:kk,:); zeros(1,N); positions2(kk+1:end,:)]
kk = 17
positions2 = [positions2(1:kk,:); zeros(1,N); positions2(kk+1:end,:)]

csvwrite('/home/aurora/Git_repos/posgradoActual/Melbourne/BMECode/NApos.csv', positions2);
...
MAPE = sum(100*abs((real_temperature-predict_temprature_BME)./real_temperature))/length(real_temperature)
RMSE = sqrt(mean((real_temperature - predict_temprature_BME).^2));
CVRMSE = RMSE/mean(real_temperature)*100
...
pred_noZero = predict_temprature_BME(find(real_temperature));
real_noZero = real_temperature(find(real_temperature));

MAPE = sum(100*abs((real_noZero-pred_noZero)./real_noZero))/length(real_noZero)
RMSE = sqrt(mean((real_temperature - predict_temprature_BME).^2));
CVRMSE = RMSE/mean(real_temperature)*100
