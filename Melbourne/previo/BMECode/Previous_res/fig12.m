...
ns = 1:99
k = 10 % fixed number of clusters
finalRes = zeros(length(ns),k);
finalTime = zeros(length(ns),k);
ssaux=1
NNN = 1200
...
kk = 4;
matrixForNA2 = [matrixForNA2(1:kk,:); zeros(1,NNN); matrixForNA2(kk+1:end,:)];
kk = 14;
matrixForNA2 = [matrixForNA2(1:kk,:); zeros(1,NNN); matrixForNA2(kk+1:end,:)];
kk = 17;
matrixForNA2 = [matrixForNA2(1:kk,:); zeros(1,NNN); matrixForNA2(kk+1:end,:)];
...
foo = foo+1;
finalRes(n,index) = RMSE
finalTime(n, index) = time2
end
...
csvwrite('fig12NAchangesNUMCL10(row=NAcol=clu).csv',finalRes) 
csvwrite('fig12NAchangesNUMCL10(row=NAcol=clu)TIME.csv',finalTime) 
