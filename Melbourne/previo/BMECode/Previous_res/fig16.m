...
temp = [];
tempN = [];
tempP = []; % temperature prior to error adition
for i= submotes_for_BME2
temp = [temp; eval(['mote_' num2str(i) '_temprature_actual'])];
tempP = [tempP; eval(['mote_' num2str(i) '_temprature_actual_prev'])];
...
realTest = tempP(test_index);
maxTrain = max(tempP(train_index));
minTrain = min(tempP(train_index));
...
%csvwrite('fig16NA10pNUMCLchangesSOFT.csv',[cluster; time; RMSE_final]) 
%csvwrite('fig16NA10pNUMCLchangesInnerRMSESOFT.csv',RMSE_inner) 