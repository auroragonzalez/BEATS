df <- read.table("../softBME2.csv", sep=",", head=F)
...
df2 = read.table("../softPMF2res.csv", sep=",", head=F)
df2[df2 == "NaN"] = NA
df2 = df2[1:79,]
...
plot(df$V1, df$RMSE,ylab ="RMSE", xlab ="Percentage of missing values"
     , main="Soft sensors temp IBRL data"#, ylim = c(4.003,4.027)
...
dev.off()
svg(filename="fig2.svg", width = 12, height = 8, pointsize = 12)
plot(df$V1, df$RMSE,ylab ="RMSE", xlab ="Percentage of missing values"
     , main="Soft sensors temp IBRL data"#, ylim = c(4.003,4.027)
