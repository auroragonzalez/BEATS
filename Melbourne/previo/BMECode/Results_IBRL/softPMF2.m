...
maxk = 20 % cambiar a 20
maxepoch=1000; % cambiar a 1000

cluster = repelem(1:maxk,1:maxk);
RMSE_inner = zeros(maxepoch, length(cluster));


finalRes = zeros(length(ns),length(cluster));
finalTime = zeros(length(ns),length(cluster));
...
nNA = nrow*ncol*n/100;  % n percent has to be NA
...
realTest = tempT(test_index);
maxTrain = max(tempT(train_index));
minTrain = min(tempT(train_index));
...
finalRes(n,:) = RMSE_final;
finalTime(n,:) = time;

end


csvwrite('softPMF2res2.csv',[cluster; finalRes]) 
csvwrite('softPMF2time2.csv',[cluster; finalTime]) 