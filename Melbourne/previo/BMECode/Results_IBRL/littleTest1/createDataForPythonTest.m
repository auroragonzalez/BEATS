...
matrixForNA=zeros(54,NNN);  % 54 sensors
motes_for_BME=[1:4 6:14 16:17 19:54];


for m = motes_for_BME
    eval(['matrixForNA(' num2str(m) ',:) = mote_' num2str(m) '_temprature_actual(1:NNN);' ]);%%%SOFT
end


csvwrite('dataHard.csv',matrixForNA) 
...
for m = motes_for_BME
    a = eval(['mote_' num2str(m) '_temprature_actual(1:NNN)']);
    x = [min(a,[],1);max(a,[],1)];
    b = bsxfun(@minus,a,x(1,:));
    b = bsxfun(@rdivide,b,diff(x,1,1));     

    eval(['matrixForNA(' num2str(m) ',:) = b;' ])
end

csvwrite('dataHardN.csv',matrixForNA) 
...
%for m = soft %%%SOFT
%rng(m); %%%SOFT
%eval(['mote_' num2str(m) '_temprature_actual = mote_' num2str(m) '_temprature_actual -0.5 + (0.5+0.5)*rand(1200,1);' ]); %%%SOFT
%end %%%SOFT
