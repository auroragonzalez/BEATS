...
deltas = 0:0.1:0.5;
RMSEns = zeros(length(deltas),1);
timens = zeros(length(deltas),1);
...
RMSE = sqrt(mean((realTest - predTest).^2));
  ...
  err_valid(epoch) = sqrt(sum((pred_out- rating).^2)/NN);
   fprintf(1, 'epoch %4i batch %4i Training RMSE %6.4f  Test RMSE %6.4f Final RMSE %6.4f  \n', ...
...
csvwrite('C:\Users\AGONZALEZVID\Desktop\Work_melb\actual\BMECode\Results_Dockland2\softPMF2deltas.csv', [RMSEns timens])
