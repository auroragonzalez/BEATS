...
f.subdatesSetLinesPlot(date1, date2)
dev.off()
...
uno = data.frame(elecF[lim]
                 , strftime(f.dateMtoR(timeF[lim]))
                 , strftime(f.dateMtoR(timeF[lim]),format="%W"))

dos = data.frame(dfx$V2[lim2]
                 , dfx$date[lim2]
                 , strftime(dfx$date[lim2],format="%W"))




sum(uno[c(23:46),]$elecF.lim.)
sum(dos[144:286,]$dfx.V2.lim2.*1/6)
...
lim = which(timeF < date1  & timeF >date2 )


plot(timeF[lim], toutF[lim], ylim = c(0,40))
lines(timeF[lim], 0.0001*elecF[lim], col="blue")
lines(timeF[lim], YY[lim], col="red")

f.rmse(YY[lim]- 0.0001*elecF[lim])
f.rmse(YY[lim]- 0.0001*elecF[lim]) / mean(0.0001*elecF[lim])

f.mae(YY[lim]- 0.0001*elecF[lim])
f.mae(YY[lim]- 0.0001*elecF[lim])/ mean(0.0001*elecF[lim])

plot(timeF[lim], 0.0001*elecF[lim], col="blue", type="l")
lines(timeF[lim], YY[lim], col="red")
...
plot(timeF[lim], constantsReal, col="black", lwd=4, type="l", ylim = c(min(constantsReal, constantsPred), max(constantsReal, constantsPred)))
lines(timeF[lim], constantsPred, col="red", lwd=4)
