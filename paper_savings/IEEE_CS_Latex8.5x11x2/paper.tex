...
Many efforts from several organisations are focused on increasing energy efficiency. This is on the interest of everybody, from particulars to governments, since energy efficiency yields to economical savings for households and companies, to reduce greenhouse gas emissions benefiting environment, helps alleviate energy poverty as it contributes to growth and more jobs.

Buildings are one of the largest consumers of primary energy and its efficiency is an important goal for the achievement of the mentioned goals.

Before implementing an energy efficiency solution, it is necessary to define the relationship which exists between energy use and the current builidng operating conditions. That is important in order to determine energy savings that could result form the implementation of a given solution. 

White box models based on physics equations have been used to model building' systems to predict whole buildings and their sub-systems behaviors, such as their energy consumption and indoor comfort since the are very useful for the understanding of the fundamental part of the model. However, currently the new paradigm of the Internet of Things allows us to count on vast amounts of data that can be used for knowledge extraction of all kinds, one can foreseen that energy prediction will not be an exception.
The new paradigm that brings large amounts of data, and the advantages on machine learning, has motivated us to test the hypothesis of this paper. This hypothesis supports that prior information that one can find on the physics of the phenomena, is now redundant due to the completeness of the data from the system.

We propose a machine learning approach and a grey box model approach to test this hypothesis. Energy consumption prediction models were created with the two approaches. The model was designed to estimate the energy consumption in normal operation state, i.e., if the  system would not have been altered by the energy efficiency intervention. %This allows us to compare the predicted for normal operating state and the actual consumption in order to quantify the energy savings.

Our black box method, which is based on a combination of statistical, machine learning models and on a time series structurisation of the data, shows better prediction accuracy than the so-called grey box methods that includes basic physical equationss.  This proves for this case that the hypothesis of this paper can be rejected i.e. also in this field a data driven approach outperforms more informed methods.
